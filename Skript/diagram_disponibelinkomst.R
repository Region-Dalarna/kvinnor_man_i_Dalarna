diag_disponibelinkomst<-function(region_vekt = "20", 
                                 ...
                                 returnera_data = TRUE,
                                 inkomst_typ = "Medianvärde, tkr", # "Finns Medelvärde",  tkr, "Medelvärde, procent av samtliga hushåll", "Medianvärde, procent av samtliga hushåll". Max 1 åt gången
                                 spara_figur = TRUE,
                                 alder = "18+ år"){ # Finns även: "18+ år" "18-29 år" "30-49 år" "50-64 år" "65+ år" "65-79 år" "66+ år" "80+ år". Max 1 åt gången){
  
  # =================================================================================================================
  # Skript som laddar hem data för disponibel inkomst från SCB och skapar ett diagram där värde för första och sista år
  # i datasettet jämförs.
  # 
  # Källa: https://www.statistikdatabasen.scb.se/pxweb/sv/ssd/START__HE__HE0110__HE0110G/TabVX4bDispInkN/
  #
  # Har uppdaterat med nya PXweb / Jon 2026-07-03
  # =================================================================================================================
  ...
  # Uttag med nya pxweb2-funktionen
  disponibel_inkomst_df <- pxweb2_hamta_data(
    tabell = "TAB1492",
    query = list(
      Region = region_vekt,
      Hushallstyp = "*",
      Alder = "*",
      ContentsCode = "Medianvärde, tkr",
      Tid = "*"
    ))
  
  # Koden nedan används för att byta namn på den sista variabeln i df (för att efterlikna tidigare hämtning med gamla PXweb)
  new_name <- unique(disponibel_inkomst_df$tabellinnehåll)
  
  disponibel_inkomst_df <- disponibel_inkomst_df %>% 
    rename(!!new_name := value) %>% 
    select(-tabellinnehåll,-region_kod) %>%
