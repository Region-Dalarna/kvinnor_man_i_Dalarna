...
```
<br>
I en nationell jämförelse har Dalarna en relativt liten andel personer med hög
utbildning. Riksgenomsnittet för åldersgruppen 25-64 år med minst en treårig eftergymnasial utbildning är 37 procent bland kvinnorna och 24 procent bland männen.
...
annat el och VVS, där kvinnor bara utgjorde ungefär `r round((forvarvsarbetande_bransch %>% filter(kön=="kvinnor",naringsgren=="Bygg",region == "Dalarna") %>% .$antal/sum(forvarvsarbetande_bransch %>% filter(naringsgren=="Bygg",region == "Dalarna") %>% .$antal))*100,0)` procent av de
förvärvsarbetande `r unique(forvarvsarbetande_bransch$år)`. Även om det har skett en ökning av andelen kvinnor inom byggbranschen, är denna ökning försumbar. Inom vård och omsorg är bilden den motsatta. Här utgjorde kvinnor ca `r round((forvarvsarbetande_bransch %>% filter(kön=="kvinnor",naringsgren=="Vård och omsorg",region == "Dalarna") %>% .$antal/sum(forvarvsarbetande_bransch %>% filter(naringsgren=="Vård och omsorg",region == "Dalarna") %>% .$antal))*100,0)` procent av andelen förvärvsarbetande `r unique(forvarvsarbetande_bransch$år)`.
Bygg samt vård och omsorg är även de gymnasieprogram som har störst övervikt
av pojkar respektive flickor. Det innebär att en framtida förändring i balansen mellan män och kvinnor inom dessa branscher inte verkar vara inom räckhåll i nuläget. Att få till en förändring av ungdomars studieval kräver troligtvis olika insatser.
