...
langtidsarbetsloshet_utbildning_df$Utbildningsniva <- factor(langtidsarbetsloshet_utbildning_df$Utbildningsniva, levels = c("Eftergymnasial utbildning 3 år eller längre","Eftergymnasial utbildning kortare än 3 år","Gymnasial utbildning 3 år","Gymnasial utbildning högst 2-årig","Förgymnasial utbildning 9 år","Förgymnasial utbildning kortare än 9 år")[6:1])

diagramtitel <- "Arbetslöshet i Dalarna (18-64 år) uppdelat på tid som arbetslös år 2021"
diagramfilnamn <- "langtidsarbetsloshet.png"

langtidsarbetsloshet <- SkapaStapelDiagram(skickad_df =langtidsarbetsloshet_utbildning_df %>% 
                                             filter(Arbetsloshet_langd == "Långtidsarbetslös",
                                                    Utbildningsniva != "Uppgift saknas"), 
                             skickad_x_var = "Arbetsloshet_langd", 
                             skickad_y_var = "Andel", 
...
mammorna, en andel som hade minskat till runt `r as.numeric(foraldrapenning_df %>% filter(Kommun == "Samtliga kommuner", Kön == "Kvinnor", År == max(År)) %>% .$Andel)` procent `r max(foraldrapenning_df$År)`.
...
jämställt är `r stri_extract_last(foraldrapenning_df %>% filter(Kommun != "Samtliga kommuner", Kön == "Män", År == max(År)) %>% filter(Andel == min(Andel)) %>% .$Kommun, regex='\\S+')`, där motsvarande uttag är ca `r as.numeric(foraldrapenning_df %>% filter(Kommun != "Samtliga kommuner", Kön == "Män", År == max(År)) %>% filter(Andel == min(Andel)) %>% .$Andel)` procent.
